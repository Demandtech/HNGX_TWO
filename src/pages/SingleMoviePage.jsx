...
import { getMovieDetail } from '../redux/movieThunk'

const SingleMoviePage = () => {
  const { id } = useParams()
  const dispatch = useDispatch()
  const { movie_detail } = useSelector((store) => store.movie)
  ...
  useEffect(() => {
    if (movie_detail) {
      setReleaseDate(getUtcData(movie_detail?.release_date))
      setGenres(getGenres(movie_detail?.genres))
      setRuntime(getTime(movie_detail?.runtime))
    }
  }, [movie_detail])
      ...
      <div className='p-5'>
        ...
        <div className='flex items-start flex-wrap'>
          <div className='text-[#404040] text-[1.2rem] mr-10 lg:w-1/2'>
            <span data-testid='movie-title'>{movie_detail?.title}</span> •{' '}
